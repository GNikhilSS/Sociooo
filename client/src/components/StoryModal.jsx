  ...
  return (
    <div className="fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4">
        <div className="w-full max-w-md">
            ...
            <div className="flex mt-4 gap-2">
                <button onClick={() => {setMode('text'); setMedia(null); setPreviewUrl(null);}} className={`flex-1 flex items-center justify-center gap-2 p-2  cursor-pointer rounded hover:scale-95 transition ${mode === 'text' ? "bg-white text-black" : "bg-zinc-800"}`}>
                    <TextIcon size={18}/> Text?
                </button>
                <label className={`flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer hover:scale-95 transition ${mode === 'media' ? "bg-white text-black" : "bg-zinc-800"}`}>
