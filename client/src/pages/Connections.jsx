    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600">Manage your network</p>
        </div>
          ...
          {dataArray.find((item)=>item.label === currentTab).value.map((user)=>(
            <div key={user._id} className="w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600">{user.bio.slice(0, 31)}...</p>
                <div className="flex max-sm:flex-col gap-2 mt-4">
                  ...
                  }
                  {
                    currentTab === 'Connections' && (
                      <button onClick={()=>navigate(`/messages/${user._id}`)} className="w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 active:scale-95 transition cursor-pointer flex items-center justify-center gap-1">
